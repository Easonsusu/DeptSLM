    ...
    try:
        with factory.begin() as session:
            ...
            if operation_type == "reconcile":
                ...
                query = query.join(
                    TrainingJob,
                    (TrainingJob.id == TrainingJobAttempt.training_job_id)
                    & (TrainingJob.department_id == TrainingJobAttempt.department_id),
                ).where(
                    TrainingJob.status == "succeeded",
                    TrainingJob.review_status.in_(("rejected", "archived")),
                    TrainingJob.reviewed_at <= cutoff,
...
def _execute(
    ...
    applied = 0
    outcomes: dict[tuple[UUID, UUID, str], bool] = {}
    ...
    for candidate in candidates:
        try:
            ...
            applied += 1
            outcomes[(candidate.training_job_id, candidate.attempt_id, candidate.surface)] = True
        except SftArtifactError:
            outcomes[(candidate.training_job_id, candidate.attempt_id, candidate.surface)] = False
    try:
        with factory.begin() as session:
            ...
            for candidate in candidates:
                ...
                if completed:
                    ...
                    item.status = "blocked"
                    item.blocked_at = now
                    item.blocked_reason_code = "artifact_ownership_mismatch"
                attempt = session.execute(
                    select(TrainingJobAttempt)
                    .where(
                        TrainingJobAttempt.department_id == department_id,
                        TrainingJobAttempt.training_job_id == candidate.training_job_id,
                        TrainingJobAttempt.publication_attempt_id == candidate.attempt_id,
                    )
                    .with_for_update()
                ).scalar_one_or_none()
                if attempt is not None and candidate.surface == "final" and completed:
            ...
            append_mutation_audit(
                ...
                action="training.job.artifact.maintenance",
