...
Each evaluation publication has a server-generated attempt UUID. External suite/result manifests are closed schemas, published through private descriptor-verified paths, and reconciled only when PostgreSQL metadata proves exact department ownership. PostgreSQL and external storage are not transactionally atomic: an orphaned external result after a crash is never retrieval or evaluation authority. `reconcile-artifacts` is dry-run by default, department-scoped, and limited to system or department administrators.
