...
Phase 8 feedback is immutable structured PostgreSQL metadata attached to the original requester's completed run. Identical canonical PUT replay is idempotent; reviewer transitions are versioned and constrained. Expired feedback becomes inaccessible before explicit authorized batch purge. Persistent audit rows may outlive purged feedback, and local Compose is not a production privacy or retention claim.
