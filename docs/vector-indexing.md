...
Claims use PostgreSQL server time, `FOR UPDATE SKIP LOCKED`, random worker UUIDs, and fresh claim/vector-attempt IDs. The claim commits before artifact or network work. Heartbeat, requeue, failure, activation, and finalization require exact worker, claim, attempt, running state, and a lease strictly in the future. Expiry is non-revivable.
...
One secret-free offline embedding subprocess loads the pinned model once per job. Batches are bounded by count and total characters; every ordered 1024-dimensional vector is validated before a content-free point is staged. No PostgreSQL transaction remains open during inference or normal Qdrant writes.

On graceful shutdown, the process group stops, the exact current attempt is cleaned, and the job requeues only if the lease is still valid. A hard crash relies on expiry; reclaim receives and cleans only the exact old attempt before rebuilding. Any model, artifact, database, Qdrant, point-count, claim, or cleanup failure prevents success.
