...
| `training-execution-worker` | Phase 14.2 execution control plane | Opt-in `training` profile; PostgreSQL, exact Phase 11 snapshot verification, `training_runs`, and private runtime IPC only; no model cache or training stack. |
| `training-runtime` | Phase 14.2 private offline trainer | Opt-in `training` profile; Linux/x86_64 one-GPU runtime with exact model-cache directory and IPC volume only; no database, network, or public port. |
