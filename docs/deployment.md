...
DeptSLM is not a production deployment. Phase 9 completed the internal evaluation runner with external immutable suites, content-free result artifacts, and PostgreSQL-backed leases. It reuses the completed Phase 7 grounded-answer boundary; it is not a public evaluation API. Phase 10 completed an isolated dataset-builder worker that writes only private external SFT dataset artifacts. Phase 11 completed a separate worker that generates immutable LlamaFactory configuration bundles only; it does not install or execute LlamaFactory, train models, or create adapters. Phase 12.0 through Phase 12.2 are completed. Phase 12.3 is the current reviewed scope: an internal, administrator-only governance API and durable metadata-only worker for explicit review, approval, promotion, rollback, retention, and deployment events. It verifies the exact registry final through a read-only mount and never loads or routes an adapter. Public vector search, conversations, history, streaming, reranking, scheduled evaluation, malware scanning, OCR, training, runtime adapter routing, production identity/storage, secrets management, backups, clustering, and production operations remain deferred. Phase 12.4 and later are not started.

Phase 12.1D, E-A, E-B, and E-C are complete; Phase 12.2 is complete and Phase 12.3 is under review, while Phase 12.4 is not started. The public adapter API remains limited to closed content-free metadata reads, evaluation metadata, and explicit governance metadata; no route exposes artifacts or generated content. The source CLI is administrator-controlled and dry-run by default. The registry, evaluation, and governance workers are internal PostgreSQL-enqueued workers; governance verifies a read-only registry final and does not load, route, reconcile, or purge. The separate maintenance profile runs E-A reconciliation, E-B purge, or E-C lifecycle release only when explicitly invoked; it has no model, Qdrant, dataset, training-job, or evaluation-runtime mount. PostgreSQL and external storage remain non-atomic.
...
| `adapter-governance-worker` | Phase 12.3 review/deployment governance worker | PostgreSQL plus the exact external `adapters/registry` final mount read-only; no model, tokenizer, Qdrant, RAG-runtime, dataset, training-job, evaluation-result, upload, extraction, or adapter-write mount. |
