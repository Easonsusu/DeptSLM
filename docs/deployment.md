...
DeptSLM is not a production deployment. Phase 9 completed the internal evaluation runner with external immutable suites, content-free result artifacts, and PostgreSQL-backed leases. It reuses the completed Phase 7 grounded-answer boundary; it is not a public evaluation API. Phase 10 completed an isolated dataset-builder worker that writes only private external SFT dataset artifacts. Phase 11 completed a separate worker that generates immutable LlamaFactory configuration bundles only; it does not install or execute LlamaFactory, train models, or create adapters. Phase 12.0 through Phase 12.1E-C are completed. Phase 12.2 is the current reviewed scope: an internal, administrator-only paired evaluation worker and private candidate runtime. It mounts only PostgreSQL, the exact evaluation suite/extracted-text inputs, the existing Qdrant and base-runtime boundaries, and external `eval_results`; it does not mount model caches, adapters, uploads, datasets, credentials, or API storage into the API. It publishes content-free numeric evidence only. Public vector search, conversations, history, streaming, reranking, scheduled evaluation, malware scanning, OCR, training, approval, promotion, runtime adapter routing, production identity/storage, secrets management, backups, clustering, and production operations remain deferred. Phase 12.3 and later are not started.

Phase 12.1D, E-A, E-B, and E-C are complete; Phase 12.2 is under review, while Phase 12.3 and 12.4 are not started. The public adapter API remains limited to closed content-free metadata reads and evaluation metadata reads; paired evaluation is administrator-only and does not expose artifacts or generated content. The source CLI is administrator-controlled and dry-run by default. The registry and evaluation workers are internal PostgreSQL-enqueued workers and do not approve, promote, load, route, reconcile, or purge. The separate maintenance profile runs E-A reconciliation, E-B purge, or E-C lifecycle release only when explicitly invoked; it has no model, Qdrant, dataset, training-job, or evaluation-runtime mount. PostgreSQL and external storage remain non-atomic.
