...
Retrieval metrics come from the authorized retrieval trace, not from whether generation later satisfies the answer contract. Invalid JSON, forged or duplicate citations, and thinking output therefore produce a failed case with `answer_contract_valid=false` and increase invalid-contract rate while retaining the safe retrieval trace. A retrieval failure has no successful trace. Generated answers and evidence remain process-local and are never stored.
