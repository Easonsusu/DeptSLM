...
Run manifests bind the department, suite, run, server-generated publication UUID, positive run attempt number, and code revision. Final deletion verifies that entire tuple and every allowlisted payload digest before removal. Staging uses a separate ownership contract because a supervised writer may die after creating its exact directory but before returning a completed artifact: PostgreSQL metadata plus the exact private UUID path, private current-service-user ownership, and one no-follow descriptor chain held through removal permit deletion of partial payloads without reading them. The content-free marker is only staging housekeeping, never cleanup authority: missing, zero-byte, truncated, partially written, and completed marker states are all recoverable. Marker and payload bytes are never parsed during partial-stage deletion. Symlinked, substituted, foreign, or unsafe stage boundaries fail closed.
