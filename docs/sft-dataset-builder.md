...
The worker uses finite PostgreSQL-server-time leases and exact worker/token/publication-attempt ownership. It does not hold database locks while reading or publishing artifacts. External storage and PostgreSQL are not transactionally atomic; exact-owned staging may be reconciled, while unknown/mismatched paths fail closed. Purge is explicit, bounded, retention-based, preserves metadata and audits, and does not delete backups. Google Drive is not a production object store or backup.
