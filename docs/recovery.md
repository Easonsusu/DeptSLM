...
| Phase 11 bundle crash/purge | Descriptor-bound attempt cleanup remains exact | Run `python -m app.admin reconcile-training-job-artifacts` or the explicit `purge-training-job-artifacts` command for the exact job | Execute the generated config as recovery | Phase 14.2 execution is a separate opt-in boundary |
| Phase 14.2 runtime disconnect or worker loss | The private runtime observes socket loss and terminates the exact process group; output remains non-authoritative | Restart the training profile and explicitly retry the execution after reviewing PostgreSQL state | Adopt output bytes or mark an execution succeeded by hand | An in-flight write cannot be fenced retroactively |
| Phase 14.2 timeout, cancellation, or unsupported hardware | The child is terminated/reaped or preflight fails closed with a fixed code; no output is retained as success | Resolve the reviewed Linux/NVIDIA/model/dependency prerequisite, then explicitly retry | Fall back to CPU/MPS/FP16/LoRA or enable downloads | Real GPU validation is opt-in and hardware is not attested |
| Phase 14.2 failed cleanup or orphan candidate bytes | Bytes remain non-authoritative and PostgreSQL never adopts them | Preserve the exact attempt namespace for an authority-driven maintenance cleanup | Move bytes into Phase 12 or delete an unknown path manually | Phase 14.3 owns publication/reconciliation |
| Phase 14.2 retained real success | Private output is retained and the exact Phase 11 job remains fenced | Keep the source retained until the reviewed Phase 14.3 release/handoff | Purge the Phase 11 job or manually release the fence | No release or adapter handoff exists in 14.2 |
