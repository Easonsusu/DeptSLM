...
After generation, the API starts a new short transaction, locks in deterministic order, reauthorizes the caller, and revalidates every cited document, extraction, indexing attempt, and chunk. A soft-deleted document, unavailable extraction, stale attempt, changed artifact, lost membership, or database failure prevents an answered success.
...
`rag.answer.start` is committed with the admitted run. `rag.answer.complete` is committed only with an applied answered or insufficient result and exact citation rows. Failures do not create a completion-success audit. PostgreSQL, Qdrant, external artifacts, and the model runtime do not share a transaction; post-generation artifact verification, final authorization, and PostgreSQL source authority reduce stale-state acceptance but do not claim distributed atomicity or an atomic filesystem/database snapshot.
