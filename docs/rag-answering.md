...
The prompt-injection defense is layered validation, not a proof that a language model cannot be manipulated. Structured user feedback does not establish correctness and is not evaluation ground truth or training data. Phase 9 reuses this exact internal production path for controlled evaluation without creating normal answer/citation rows or persisting generated content. Evaluation keeps an authorized retrieval trace when a later generation-contract check fails, but never persists the generated answer or evidence. Exact match, character F1, and citation overlap are limited regression metrics, not semantic or entailment proofs. Production rate limiting, TLS, secrets management, model distribution, observability, reconciliation, and hardware sizing remain deferred. See [prompt-injection-boundary.md](prompt-injection-boundary.md), [citation-model.md](citation-model.md), [rag-feedback.md](rag-feedback.md), [evaluation-runner.md](evaluation-runner.md), and [rag-runtime.md](rag-runtime.md).
