...
Claims use PostgreSQL server time, exact worker and claim UUIDs, non-revivable leases, department-first locking, bounded heartbeats, and fresh tokens after reclaim. Every claim carries a positive `attempt_number`; that number, the publication UUID, department, suite, run, and code revision are bound into the closed private result manifest. Suite reading, each case, final ground-truth verification, result staging, and publication run in killable child process groups with an operation deadline. The parent renews the exact live claim while work is active; timeout, cancellation, shutdown, claim loss, or a heartbeat database failure terminates and reaps the child group. A stale worker cannot publish or finalize; shutdown leaves work reclaimable, while cancellation becomes cancelled. A replacement removes only the exact stale attempt staging or final artifact.
