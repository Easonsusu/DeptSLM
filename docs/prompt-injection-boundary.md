...
A shared visible-text policy rejects every Unicode `Cf` format control, the combining grapheme joiner U+034F, surrogates, NUL, disallowed C0/C1 controls, and Unicode noncharacters in questions, evidence, and answers. This covers bidi controls, zero-width controls, invisible operators U+2061–U+2064, soft hyphen, and Unicode tag controls. Variation selectors, ordinary combining accents, and ordinary emoji remain valid. Citation tokens must be exact ASCII `[S1]` through `[S8]`; the focused lexer rejects paired or dangling lowercase, whitespace, full-width/mixed-bracket, leading-zero, out-of-range, hidden-character, and long malformed source-like forms while leaving unrelated bracket prose and normal parentheses alone. Unsafe original filenames use complete deterministic visible Unicode escapes within a fixed bound without modifying the database value.
