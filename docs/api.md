...
All five active same-department roles may submit structured feedback only for their own completed run. A PUT accepts `sentiment`, reviewed `reason_codes`, and exact public citation IDs `S1` through `S8`; extra keys and arbitrary strings fail validation. The PUT body is limited to 4,096 streamed bytes before JSON decoding. First creation returns 201, an identical canonical replay returns 200 without mutation or audit, and a conflicting immutable replacement returns 409. The owner GET requires current active membership and hides expired or foreign feedback with 404.

The reviewer queue and detail/mutation routes require active same-department `system_admin`, `department_admin`, or `instructor` membership. The queue supports exact status/sentiment filters, a service-enforced limit of 1 through 100, oldest-first ordering, and an opaque filter-bound cursor. Review PATCH has a 2,048-byte pre-JSON limit, supports only documented forward transitions and exact resolution identifiers, and requires a strict non-boolean integer `expected_version`; stale or invalid transitions return 409. Malformed transport/JSON returns 400, oversized feedback bodies return 413, and schema failures return 422. Read responses use one complete PostgreSQL statement snapshot, so concurrent expiry or purge returns a full eligible record or safe 404, never partial reasons or sources. Authentication failures retain the 401 Bearer challenge; authorization 403 responses do not.
...
### Phase 12 adapter registry and evaluation metadata

Phase 12.1D implements these two department-scoped, PostgreSQL-backed metadata
reads:
...
Phase 12.2 implements administrator-only paired evaluation metadata operations
for one exact validated adapter:

- `POST /departments/{department_id}/adapters/{adapter_id}/evaluations`
- `GET /departments/{department_id}/adapters/{adapter_id}/evaluations`
- `GET /departments/{department_id}/adapters/{adapter_id}/evaluations/{evaluation_id}`
- `POST /departments/{department_id}/adapters/{adapter_id}/evaluations/{evaluation_id}/cancel`
