...
The worker receives only PostgreSQL and `training_datasets`; it has no model, tokenizer, LlamaFactory package, Hugging Face token, Qdrant, RAG runtime, upload, extraction, evaluation, adapter, or cloud configuration. It never runs the generated configuration. Staging verification, marker transition, rename, post-rename verification, and final source reauthorization retain independent lease checkpoints. The parent renews a lease before, never inside, the final locked transaction; that transaction uses PostgreSQL server time for the final ownership check. Purge uses a durable review-fencing reservation: each succeeded job has one exact final owner and physical final surface, while attempts retain only their distinct stage cleanup surfaces. Stages must complete before final-deletion authorization; a blocked stage leaves the final intact and terminalizes the retryable reservation history. PostgreSQL and external storage remain non-atomic: an artifact without committed succeeded PostgreSQL authority is not eligible for review or future adapter use.
