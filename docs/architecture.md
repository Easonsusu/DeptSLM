...
flowchart TB
    ...
    subgraph AdapterBoundary["Phase 12 adapter governance and runtime (completed)"]
...
## Phase 12 adapter registry, evaluation, governance, and runtime boundary (completed)
...
and bounded header validation without loading a model, tokenizer, adapter, or
tensor payload. It records no training provenance and does not make an external
artifact trusted. The diagram separates an untrusted external training
environment from the DeptSLM application. The Phase 12.1C worker consumes one
exact approved Phase 11 bundle and captured Phase 10 authority, then produces
untrusted immutable registry files. Phase 12 records only a verified governance
lineage, a declared external training association, and verified artifact
compatibility; it cannot prove exact bundle or dataset use, declared
LlamaFactory execution, declared training steps, or unmodified weight
production. Operator attestations remain untrusted, and stronger provenance
...
- Phase 12.3 adapter review, registry lifecycle, and deployment governance are
  complete; Phase 12.4 immutable runtime routing is complete
- Production topology, secrets, observability, backup, and disaster recovery

## Roadmap v2 Phase 14.0 training execution contract

Roadmap v1 Phase 0–13 is complete. Phase 14.0 is design only: Phase 11 still
creates immutable reviewable LlamaFactory bundles and no DeptSLM component
executes training. A future control plane will freeze one approved same-
department Phase 11 authority, create a verified five-file input snapshot, and
...
Queued/running executions will fence Phase 11 purge, and retained succeeded
output will retain its exact upstream authority until an explicit reviewed
release or Phase 12 handoff. Phase 14.0 adds no migration, service, worker,
endpoint, dependency, Compose entry, or runtime files; implementation starts
only after review in Phase 14.1.
