    ...
    def generate(self, payload: dict[str, Any]) -> dict[str, Any]:
            ...
            if self.provider == "real":
                try:
                    self.copy = verify_and_copy_adapter(
                        ...
                        registry_publication_attempt_id=target[
                    ...
                    self.model = load_candidate_model(
                        self.copy, self.data_dir / "model_cache"
                    )
        ...
        if self.provider == "fake":
            ...
            digest = hashlib.sha256(
                f"candidate:{key}:{seed}:{question}".encode()
            ).hexdigest()[:8]
            value = {
                "status": "answered",
...
def _target(payload: dict[str, Any], *, generation: bool) -> dict[str, Any]:
    ...
    if (
        ...
        raise CandidateChildError("invalid_request")
    if generation and (
        not {"question", "evidence", "prompt_version", "answer_contract_version"}
        <= set(payload)
    ...
    for name in _TARGET_FIELDS:
        ...
        if name.endswith("_sha256"):
            ...
            raise CandidateChildError("invalid_request")
        elif name == "registry_attempt_number" and (
