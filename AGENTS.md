...
- Result staging and publication require a server-generated publication attempt UUID in a closed, content-free manifest. The final success transaction must never rename or write external artifacts. Reclaim and reconciliation may delete only a descriptor-verified final artifact whose department, run or suite, and exact attempt UUID match non-succeeded PostgreSQL metadata; unknown, malformed, mismatched, or succeeded artifacts remain untouched.
