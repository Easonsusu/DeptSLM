...
- Result staging and publication require a server-generated publication attempt UUID and exact positive run attempt number in a closed, content-free manifest. The final success transaction must never rename or write external artifacts. Reclaim and reconciliation may delete only descriptor-verified staging or final artifacts whose department, run or suite, exact attempt UUID, and (for runs) attempt number and code revision match non-succeeded PostgreSQL metadata; unknown, malformed, mismatched, or succeeded artifacts remain untouched. Reconciliation must register durable content-free batch ownership before deletion, terminalize cleaned metadata so it cannot starve later recovery, and finalize one resumable batch audit without deleting backups or audit history.
