...
- The Phase 11 bundle worker receives only PostgreSQL metadata and `training_datasets`; it must not receive model caches, adapters, Qdrant, RAG runtime settings, uploads, extraction data, evaluation data, Hugging Face tokens, proxies, cloud credentials, or application-auth secrets. Its fixed child receives only exact source/staging descriptors and closed content-free metadata; content never crosses IPC.
